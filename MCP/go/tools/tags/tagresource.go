...
func CreateTagresourceTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_tags_ResourceArn",
		mcp.WithDescription("Assigns one or more tags (key-value pairs) to the specified EventBridge Scheduler resource. You can only assign tags to schedule groups."),
		mcp.WithString("ResourceArn", mcp.Required(), mcp.Description("The Amazon Resource Name (ARN) of the schedule group that you are adding tags to.")),
		mcp.WithArray("Tags", mcp.Required(), mcp.Description("Input parameter: The list of tags to associate with the schedule group.")),
	)
